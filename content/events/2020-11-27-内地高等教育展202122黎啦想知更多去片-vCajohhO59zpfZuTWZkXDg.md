+++
date = '2020-11-27T16:44:26+08:00'
draft = false
...
var __INLINE_SCRIPT__ = (function () {
    ...
    var IS_AUTHOR_SCENE_PHPTO = [338, 339, 340];
    var IS_AUTHOR_SCENE_SUBSCRIBE = [305, 306, 340];
    ...
    function isAuthorScene(scene, bizType) {
      var _a;
      if (scene === void 0) {
        scene = window.source;
      }
      ...
      if (window.previewtype === 1) return true;
      if (bizType * 1 === 2 && IS_AUTHOR_SCENE_PHPTO.includes(scene * 1)) return true;
      if (bizType * 1 === 1 && IS_AUTHOR_SCENE_SUBSCRIBE.includes(scene * 1)) return true;
      return false;
    }
    function getIsAuthor(cb, bizuin, scene, checkScene, needCheckBiz, bizType) {
      var _a;
      if (bizuin === void 0) {
        bizuin = window.biz;
      }
      ...
      if (checkScene && !isAuthorScene(scene, bizType)) return cb(false);
    ...
    var isPraiseCgi = function isPraiseCgi(url) {
      return /^\/mp\/appmsg_comment\?action=(getcomment|getcommentreply|like_reply|likecomment)/.test(url);
    };
    function joinUserArticleRole(url, notJoin, cb) {
      ...
      if (notJoin) {
        ...
        }, bizuin, scene, !isPraiseCgi(url), needCheckBiz);
      }
    }
      ...
      Url.joinUserArticleRole(_url, !!obj.notJoinUrl, function (url) {
        ...
        if (ajaxScope && !obj.pureHttp) {
          ...
          JSAPI.invoke(Device.os.pc ? 'H5ExtTransfer' : 'webTransfer', params, function (res) {
            ...
            try {
              Log.log("ajax transfer, params: ".concat(JSON.stringify({
                reqUrl: reqUrl,
                type: type,
                data: data
            ...
            if (status >= 200 && status < 400) {
              ...
              JSAPI.invoke('request', {
                ...
                try {
                  Log.log("ajax retry request, params: ".concat(JSON.stringify({
                    reqUrl: reqUrl,
                    type: type,
                    data: data
                  }), ", res: ").concat(JSON.stringify(retryRes || {})));
                } catch (err) {
                  console.error(err);
                }
                if (isTimeout) return;
...
;
    ...
    if (!window.__second_open__) {
      var videoContentNoEncode = window.a_value_which_never_exists || '內地高等教育展又黎啦！ 2021學年有127間內地高校參與文憑試招生計劃，只需要文憑試中、英、數、通考到3322就可以選擇去內地唔同大學升讀！想知道有邊D高校？內地高校嘅學科學術優勢？內地港生嘅生活體驗？ 12月5號至11號記得留意2020內地高等教育展啦！我們等緊你！';
      ...
      var desc = window.a_value_which_never_exists || '內地高等教育展又黎啦！ 2021學年有127間內地高校參與文憑試招生計劃，只需要文憑試中、英、數、通考到3322就可以選擇去內地唔同大學升讀！想知道有邊D高校？內地高校嘅學科學術優勢？內地港生嘅生活體驗？ 12月5號至11號記得留意2020內地高等教育展啦！我們等緊你！';
